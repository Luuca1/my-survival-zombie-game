...
function createInitialState() {
    return {
        ...
        level: 1, activeBossId: null, nextBossDistance: 1000 
    };
}
...
function spawnZombie(x, y, typeKey, levelMultiplier) {
...
function generateChunkIfNeeded(playerX, playerY) {
    ...
    for (let offsetX = -1; offsetX <= 1; offsetX++) {
        for (let offsetY = -1; offsetY <= 1; offsetY++) {
            ...
            // Geração de Consumíveis e Armaduras no Mapa
            ...
            if (seededRandom(chunkSeed++) > 0.8) state.items[generateId()] = { x: cx * GAME.CHUNK_SIZE + seededRandom(chunkSeed++) * GAME.CHUNK_SIZE, y: cy * GAME.CHUNK_SIZE + seededRandom(chunkSeed++) * GAME.CHUNK_SIZE, type: 'bandage' };
            ...
            for (let i = 0; i < numZombies; i++) {
                ...
                if (!tooClose) {
                    ...
                    spawnZombie(zx, zy, type, levelMult);
                }
            }
        }
    }
...
function dropItemFromZombie(x, y, isBoss) {
...
wss.on('connection', (ws) => {
    if (state.gameOver || Object.keys(state.players).length === 0) state = createInitialState();
    const playerId = generateId();
    ws.send(JSON.stringify({ type: 'init', id: playerId }));

    state.players[playerId] = {
        id: playerId, name: 'Sobrevivente',
    ...
    ws.on('message', (message) => {
        try {
            ...
            if (!player) return;
            if (player.isDead && data.type !== 'move' && data.type !== 'setName') return; 

            const now = Date.now();

            switch (data.type) {
                case 'setName': player.name = data.name.substring(0, 15); break;
                ...
                case 'shoot':
                    ...
                    if (player.ammo > 0 && now - player.lastShot >= wpn.cd) {
                        ...
                        state.bullets[generateId()] = {
                            ...
                            owner: playerId, life: 80, damage: wpn.damage, piercing: wpn.piercing, hitList: [] 
                        };
                    }
                    break;
                    ...
                    for (const zid in state.zombies) {
                        ...
                        if (getDist(player.x, player.y, z.x, z.y) <= meleeRange) {
                            ...
                            if (Math.abs(zAngle - player.angle) < 1.0) {
                                z.hp -= meleeDamage;
                                if (z.hp <= 0) handleZombieDeath(zid, z);
                            }
                        }
                    }
                ...
                case 'revive': 
                    if (player.medkits > 0) {
                        ...
                        for (const pid in state.players) {
                            if (pid !== playerId && state.players[pid].isDead && getDist(player.x, player.y, state.players[pid].x, state.players[pid].y) < minDist) {
    ...
    ws.on('close', () => delete state.players[playerId]);
});

function handleZombieDeath(zid, z) {
    const isBoss = (z.type === 'boss');
    dropItemFromZombie(z.x, z.y, isBoss);
    ...
    if (state.kills % 30 === 0) {
        ...
        for (const pid in state.players) {
            if (!state.players[pid].isDead) {
                state.activeBossId = spawnZombie(state.players[pid].x + 300, state.players[pid].y + 300, 'boss', 1 + (state.level*0.5));
                break;
            }
        }
    }
...
setInterval(() => {
    ...
    if (currentMaxDist >= state.nextBossDistance && furthestPlayer && !state.activeBossId) {
        ...
        state.activeBossId = spawnZombie(furthestPlayer.x + 300, furthestPlayer.y + 300, 'boss', bossLevel);
